...
int initREData(const CA::Grid&  GRID, const RainEvent& re, REData& redata)
{
  ...
  else
  {
    ...
    if(re.area.size()==4)
    {
      redata.box_area = CA::Box::create(GRID, re.area[0], re.area[1], re.area[2], re.area[3]);
    }
    // This is new. The are could have been identified as a zone, i.e. x,y,w,h/
    if(re.zone.size()==4)
    {
      // Compute the given box area from the zone.
      CA::Point     tl( CA::Point::create(GRID, re.zone[0], re.zone[1]) );
      CA::Unsigned  w = static_cast<CA::Unsigned>( std::ceil(re.zone[2] / GRID.length()) );
      CA::Unsigned  h = static_cast<CA::Unsigned>( std::ceil(re.zone[3] / GRID.length()) );
      
      redata.box_area = CA::Box(tl.x(),tl.y(),w,h);
    }
  }

  redata.grid_area = 0.0;
  redata.volume    = 0.0;
